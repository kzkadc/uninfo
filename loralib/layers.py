...
class LoRALayer():
    ...
    def add_lora_data(self):
        ...
        for param_name, lora_name in self.params_with_lora.items():
            eval(
                f'self.{param_name}').data += self.merge_BA(param_name) * self.scaling
...
class LinearLoRA(nn.Linear, LoRALayer):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        if self.r > 0 and not self.merged:
            lora_adjustment = torch.matmul(x, self.merge_BA(
                'weight').transpose(0, 1)) * self.scaling
...
class Conv3d(nn.Conv3d, LoRALayer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        nn.Conv3d.__init__(self, in_channels, out_channels,
                           kernel_size, **kwargs)
        LoRALayer.__init__(self, r=r, lora_alpha=lora_alpha)
...
class PlainMultiheadAttentionLoRA(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.proj = nn.Linear(self.embed_dim, self.embed_dim,
                              bias=existing_mha.out_proj.bias is not None)
        ...
        with torch.no_grad():
            ...
            self.v_proj.weight.data.copy_(
                existing_weight[2 * self.embed_dim:, :])
            if existing_bias is not None:
    ...
    def forward_module(
        ...
        if attn_mask is not None:
            ...
            if attn_mask.dim() == 2:
                ...
                raise RuntimeError(f"attn_mask's dimension"
                                   f" {attn_mask.dim()} is not supported")
        ...
        k = k.view(src_len, bsz * self.num_heads,
                   self.head_dim).transpose(0, 1)
        v = v.view(src_len, bsz * self.num_heads,
                   self.head_dim).transpose(0, 1)
        src_len = k.size(1)
        ...
        attn_output = attn_output.permute(2, 0, 1, 3) \
            .contiguous().view(bsz * tgt_len, embed_dim)
...
class MergedLinear(nn.Linear, LoRALayer):
    ...
    def __init__(
        ...
        r: int = 0,
        lora_alpha: int = 1,
        enable_lora: list[bool] | None = None,
        fan_in_fan_out: bool = False,
        **kwargs
    ):
